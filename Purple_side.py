...
def detect_leftright(detected_sensor_list):
    if len(detected_sensor_list) == 0:
        pass
    elif detected_sensor_list[0] == 0 and signal_purple_front_s[0][0][7] > 0.1:
        env.set_actions(purple,np.array([(0,0,0),(0,0,0)]))
        env.step()
    else:
        if detected_sensor_list[-1] % 2 == 0: # even number
            for i in range(2):
                env.set_actions(purple,np.array([(0,2,0),(0,2,0)])) # left
                env.step()
            if signal_purple_front_s[0][2][0] == 1:
                if signal_purple_front_s[0][2][7] < 0.2:
                    for i in range(3):
                        env.set_actions(purple,np.array([(1,0,0),(1,0,0)]))
                        env.step()
                    for i in range(3):
                        env.set_actions(purple,np.array([(2,0,0),(2,0,0)]))
                        env.step()
            elif signal_purple_front_d[0][2][0] == 1:
                if signal_purple_front_d[0][2][7] < 0.1:
                    for i in range(3):
                        env.set_actions(purple,np.array([(1,0,0),(1,0,0)]))
                        env.step()
                    for i in range(3):
            ...
            for i in range(2):
                env.set_actions(purple,np.array([(0,1,0),(0,1,0)])) # right
                env.step()
            if signal_purple_front_s[0][1][0] == 1:
                if signal_purple_front_s[0][1][7] < 0.2:
                    for i in range(2):
                        env.set_actions(purple,np.array([(1,0,0),(1,0,0)]))
                        env.step()
                    for i in range(2):
                        env.set_actions(purple,np.array([(2,0,0),(2,0,0)]))
                        env.step()
            elif signal_purple_front_d[0][1][0] == 1:
                if signal_purple_front_d[0][1][7] < 0.1:
                    for i in range(2):
                        env.set_actions(purple,np.array([(1,0,0),(1,0,0)]))
                        env.step()
                    for i in range(2):
...
def move_to_center(signal_purple_back_d, signal_purple_front_d, signal_purple_front_s, signal_purple_back_s):
    if signal_purple_back_d[0][1][1] == 1 and signal_purple_back_d[0][2][1] == 1:
        # print("success")
        ranges = signal_purple_back_d[0][1][7] - signal_purple_back_d[0][2][7]
        if ranges < 0:
            env.set_actions(purple,np.array([(0,1,0),(0,1,0)])) # right
            env.step()
        elif ranges > 0:
            env.set_actions(purple,np.array([(0,2,0),(0,2,0)])) # left
            env.step()
    elif signal_purple_back_d[0][1][3] == 1 and signal_purple_back_d[0][2][1] == 1:
        env.set_actions(purple,np.array([(0,1,0),(0,1,0)])) # right
        env.step()
    elif signal_purple_back_d[0][1][1] == 1 and signal_purple_back_d[0][2][3] == 1:
        env.set_actions(purple,np.array([(0,2,0),(0,2,0)])) # left
        env.step()
    else:
        if signal_purple_front_d[0][10][3] == 1 and signal_purple_front_d[0][9][3] == 1:
            if signal_purple_front_d[0][10][7] < signal_purple_front_d[0][9][7]:
                env.set_actions(purple,np.array([(0,1,0),(0,1,0)])) # right
                env.step()
            else:
                env.set_actions(purple,np.array([(0,2,0),(0,2,0)])) # left
                env.step()
        elif signal_purple_front_d[0][8][3] == 1 and signal_purple_front_d[0][7][3] == 1:
            if signal_purple_front_d[0][8][7] < signal_purple_front_d[0][7][7]:
                env.set_actions(purple,np.array([(0,1,0),(0,1,0)])) # right
                env.step()
            else:
                env.set_actions(purple,np.array([(0,2,0),(0,2,0)])) # left
                env.step()
        elif signal_purple_front_d[0][6][3] == 1 and signal_purple_front_d[0][5][3] == 1:
            if signal_purple_front_d[0][6][7] < signal_purple_front_d[0][5][7]:
                env.set_actions(purple,np.array([(0,1,0),(0,1,0)])) # right
                env.step()
            else:
                env.set_actions(purple,np.array([(0,2,0),(0,2,0)])) # left
            ...
            env.set_actions(purple,np.array([(0,random.randint(1,3),0),(0,random.randint(1,3),0)])) # stop
            env.step()


def find_its_place(signal_purple_back_d, signal_purple_front_d, signal_purple_front_s, signal_purple_back_s):
    if signal_purple_back_d[0][0][1] == 1 and 0.15 > signal_purple_back_d[0][0][7] > 0.079: #0.079828
        lst = front_sensor(signal_purple_front_d, 0)
        lit = front_sensor(signal_purple_front_s, 0)

        if len(lst) == 0:
            move_to_center(signal_purple_back_d, signal_purple_front_d, signal_purple_front_s, signal_purple_back_s)
            print("move to center")
        if len(lit) != 0:
            if len(lit) == 0:
                pass
            elif lit[0] == 0 and signal_purple_front_s[0][0][7] > 0.1:
                env.set_actions(purple,np.array([(0,0,0),(0,0,0)]))
                env.step()
            else:
            ...
            if len(lst) == 0:
                pass
            elif lst[0] == 0 and signal_purple_front_s[0][0][7] > 0.1:
                env.set_actions(purple,np.array([(0,0,0),(0,0,0)]))
                env.step()
            else:
    ...
    # 등치기
    elif signal_purple_back_d[0][0][1] == 1 and signal_purple_back_d[0][0][7] >= 0.15:
        print("out of bound")
        for i in range(2):
            env.set_actions(purple,np.array([(0,0,0),(2,0,0)]))
            env.step()
        if signal_purple_back_d[0][0][7] < 0.001:
            env.set_actions(purple,np.array([(0,0,0),(1,0,0)]))
            env.step()
        elif signal_purple_back_d[0][0][7] > 0.001:
            for i in range(2):
                env.set_actions(purple,np.array([(0,0,0),(2,0,0)]))
                env.step()
            env.set_actions(purple,np.array([(0,0,0),(1,0,0)]))
        ...
        move_to_center(signal_purple_back_d, signal_purple_front_d, signal_purple_front_s, signal_purple_back_s)
        print(signal_purple_back_d[0]) # sensor 0, 1, 2 -> wall
...
def team9(behaviour_name_1, behaviour_name_2):
    ...
    find_its_place(signal_purple_back_d, signal_purple_front_d, signal_purple_front_s, signal_purple_back_s)
